...
export default function ScoreContainer({ colorSets, onClose }) {
  ...
  return (
    <div>
      <motion.div
        className="absolute overflow-scroll py-10 inset-0 flex items-center justify-center bg-gradient-to-br from-indigo-300 to-purple-500 bg-opacity-50 backdrop-blur-sm z-50"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      >
        <motion.div
          className="bg-white shadow-lg rounded-xl p-6 w-[80%] h-[80vh] lg:h-auto overflow-scroll hideOverflow lg:w-full max-w-md md:max-w-lg"
          initial={{ scale: 0.8 }}
          animate={{ scale: 1 }}
          exit={{ scale: 0.8 }}
        >
